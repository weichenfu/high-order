...
def main(N):
    ...
    np.savetxt('4_dimensional_grid.txt', n_dimensional_grid, delimiter=',', header=str(n_dimensional_grid.shape))
...
def euler_method(y0, k, k_s, xmax=200, node=20000):
...
def main(N):
    n_dimensional_grid = np.loadtxt('4_dimensional_grid.txt', delimiter=',')
    ...
    for i in range(a):
        ...
        if np.all(np.abs(np.mean(k / N * np.sum(np.sin(results[:, np.newaxis] - results), axis=0) - k_s * np.sin(2 * results)))<0.0000001 and contains_zero==0 and contains_pi==0): #### other equilibrium
            ...
            # print(min_number)
            if max_number[0]==max_number[1] and min_number[0]==min_number[1] and abs(np.sum(np.cos(results),axis=0)) <= delta:
                count6 = count6 + 1
                # print(results)
